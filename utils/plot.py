...
def draw_curve(y1, y2=None,figPath = None):
    figPath = figPath + '/figs'
    ...
    if not os.path.exists(figPath):
        os.makedirs(figPath)
    plt.savefig(figPath+'/acc.jpg')
    plt.savefig(figPath+'/acc.eps')

def draw_loss(y1, y2=None,figPath = None):
    figPath = figPath + '/figs'
    x1 = [i for i in range(len(y1))]
    # y1 = smooth(y1, weight=0.95)
    plt.plot(x1, y1, color='b', label='train')
    ...
    if not os.path.exists(figPath):
        os.makedirs(figPath)
    plt.savefig(figPath+'/loss.jpg')
    plt.savefig(figPath+'/loss.eps')
...
if __name__ == '__main__':
    ...
    val = [0]
    sum_train_acc = 0
    with open('/home/jwk/Project/remoteSensing/Remote-Sensing-Image-Classification/pretrainModel/ResNet_VAE_2023-03-08-21-18-52/log.txt') as f:
    ...
    draw_loss(val[:81], train[:81],figPath = '/home/jwk/Project/remoteSensing/Remote-Sensing-Image-Classification/pretrainModel/ResNet_VAE_2023-03-08-21-18-52')
